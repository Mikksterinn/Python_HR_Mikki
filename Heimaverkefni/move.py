FIELD_RANGE = 10 # The maximum range of the string of "x's" and the singular "o".
...
initial_question = 'Input a position between 1 and ' + str(FIELD_RANGE) + ': ' # Constucting the first question the user is asked using the FIELD_RANGE constant.
coordinate = int(input(initial_question)) # The user input for the initial o_coordinate.
...
while movement == 'l' or movement == 'r':
    if movement == 'r' and coordinate < FIELD_RANGE: # The "and" statement is there to make sure the "o" doesn't go outside the range of the string.
